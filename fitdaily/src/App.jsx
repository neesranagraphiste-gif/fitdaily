...
const WORKOUT_PROGRAMS = {
  ...
  vendredi: { label: "Vendredi", focus: "Full Body", emoji: "🏋️", exercises: [
    ...
    { name: "Bear crawl", sets: 3, reps: "30s", rest: 45, calories: 10, desc: "A 4 pattes, avancer en gardant les genoux levés" },
    { name: "Etirements", sets: 1, reps: "5min", rest: 0, calories: 3, desc: "Bien s'étirer après l'effort" },
  ]},
  samedi: { label: "Samedi", focus: "Active Recovery", emoji: "🧘", exercises: [
    ...
    { name: "Etirements complets", sets: 1, reps: "10min", rest: 0, calories: 15, desc: "Toutes les grandes chaînes musculaires" },
...
const DEFAULT_FOODS = [
  { id: "d1", name: "Oeufs (1)", cal: 70, p: 6, g: 0.5, l: 5, unit: "unité", custom: false },
  ...
  { id: "d5", name: "Banane (1)", cal: 95, p: 1.2, g: 23, l: 0.3, unit: "unité", custom: false },
  ...
  { id: "d11", name: "Pomme (1)", cal: 80, p: 0.4, g: 21, l: 0.2, unit: "unité", custom: false },
  ...
  { id: "d14", name: "Avocat (1/2)", cal: 120, p: 1.5, g: 6, l: 11, unit: "unité", custom: false },
...
export default function SportApp() {
  ...
  const calBurned = Object.keys(completedSets).reduce((sum, key) => {
    ...
    if (parts[0] === todayKey) {
      const ex = WORKOUT_PROGRAMS[parts[0]]?.exercises[parseInt(parts[1])];
  ...
  useEffect(() => {
    ...
    else if (timerSeconds === 0 && timerRunning) { setTimerRunning(false); showNotif("Repos termine ! Prochain set !"); }
  ...
  const exProgress = (day, exIdx, total) => { let d = 0; for (let i = 0; i < total; i++) if (isSetDone(day, exIdx, i)) d++; return d; };
  const dayProgress = (day) => {
    const prog = WORKOUT_PROGRAMS[day];
    if (!prog.exercises.length) return 100;
    let total = 0, done = 0;
    prog.exercises.forEach((ex, ei) => { total += ex.sets; for (let s = 0; s < ex.sets; s++) if (isSetDone(day, ei, s)) done++; });
  ...
  const saveCustomProduct = () => {
    ...
    showNotif(`"${product.name}" enregistre dans votre base !`);
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#0a0a0f 0%,#0f1a2e 50%,#0a0f1a 100%)", fontFamily: "'DM Sans','Segoe UI',sans-serif", color: "#e8eaf6", overflowX: "hidden" }}>
      ...
      {notification && <div style={{ position: "fixed", top: 20, left: "50%", transform: "translateX(-50%)", background: "linear-gradient(135deg,#00d4aa,#0099ff)", color: "#fff", padding: "12px 24px", borderRadius: 50, fontWeight: 700, fontSize: 14, zIndex: 9999, boxShadow: "0 8px 32px rgba(0,212,170,0.4)", whiteSpace: "nowrap" }}>{notification}</div>}
      ...
      {showAddProduct && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 8888, display: "flex", alignItems: "flex-end" }}>
          <div style={{ width: "100%", background: "linear-gradient(180deg,#0f1a2e,#0a0a0f)", borderRadius: "24px 24px 0 0", padding: 24, paddingBottom: 40, maxHeight: "92vh", overflowY: "auto" }}>
            ...
            <div style={{ fontSize: 12, color: "#7c83a0", marginBottom: 20 }}>Reportez les valeurs de l'etiquette nutritionnelle (generalement pour 100g ou par portion).</div>

            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                <div style={{ fontSize: 11, color: "#7c83a0", marginBottom: 6 }}>Nom du produit *</div>
                <input value={newProduct.name} onChange={e => setNewProduct(p => ({ ...p, name: e.target.value }))} placeholder="Ex: Skyr Lidl, Barre proteinee Quest..." style={inp} />
              </div>

              <div>
                <div style={{ fontSize: 11, color: "#7c83a0", marginBottom: 8 }}>Unite de reference (indiquee sur l'etiquette)</div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {["100g", "100ml", "1 portion", "1 unite"].map(u => (
                    <button key={u} onClick={() => setNewProduct(p => ({ ...p, unit: u }))} style={{ padding: "8px 14px", borderRadius: 10, cursor: "pointer", fontSize: 12, fontWeight: 600, background: newProduct.unit === u ? "linear-gradient(135deg,#00d4aa,#0099ff)" : "rgba(255,255,255,0.07)", border: "none", color: newProduct.unit === u ? "#fff" : "#7c83a0" }}>{u}</button>
                  ))}
                </div>
              </div>

              <div>
                <div style={{ fontSize: 11, color: "#7c83a0", marginBottom: 6 }}>Calories (kcal) *</div>
                <input type="number" min="0" value={newProduct.cal} onChange={e => setNewProduct(p => ({ ...p, cal: e.target.value }))} placeholder="Ex: 65" style={inp} />
              </div>

              <div>
                <div style={{ fontSize: 11, color: "#7c83a0", marginBottom: 8 }}>Macronutriments (g) — facultatif mais recommande</div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
                  {[{ label: "Proteines", key: "p", ph: "Ex: 10" }, { label: "Glucides", key: "g", ph: "Ex: 4" }, { label: "Lipides", key: "l", ph: "Ex: 0.2" }].map(f => (
                    <div key={f.key}>
                      <div style={{ fontSize: 10, color: "#7c83a0", marginBottom: 6 }}>{f.label}</div>
                      <input type="number" min="0" step="0.1" value={newProduct[f.key]} onChange={e => setNewProduct(p => ({ ...p, [f.key]: e.target.value }))} placeholder={f.ph} style={{ ...inp, fontSize: 13 }} />
                    </div>
                  ))}
                </div>
              </div>

              {newProduct.name && newProduct.cal && (
                <div style={{ background: "rgba(0,212,170,0.08)", border: "1px solid rgba(0,212,170,0.25)", borderRadius: 12, padding: 14 }}>
                  <div style={{ color: "#00d4aa", fontWeight: 700, fontSize: 12, marginBottom: 6 }}>Apercu du produit</div>
                  <div style={{ fontWeight: 700 }}>{newProduct.name}</div>
                  <div style={{ fontSize: 12, color: "#7c83a0", marginTop: 4 }}>{newProduct.cal} kcal / {newProduct.unit} — P:{newProduct.p||0}g G:{newProduct.g||0}g L:{newProduct.l||0}g</div>
                </div>
              )}

              <button onClick={saveCustomProduct} disabled={!newProduct.name || !newProduct.cal} style={{ padding: 16, borderRadius: 14, fontWeight: 800, fontSize: 15, cursor: newProduct.name && newProduct.cal ? "pointer" : "not-allowed", background: newProduct.name && newProduct.cal ? "linear-gradient(135deg,#00d4aa,#0099ff)" : "rgba(255,255,255,0.08)", border: "none", color: newProduct.name && newProduct.cal ? "#fff" : "#4a4f6a", marginTop: 4 }}>
                Enregistrer dans ma base de donnees
              </button>
            </div>
          </div>
        </div>
      ...
      <div style={{ display: "flex", background: "rgba(255,255,255,0.03)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        {[{ key:"today",label:"Aujourd'hui",icon:"🏠"},{key:"week",label:"Programme",icon:"📅"},{key:"calories",label:"Nutrition",icon:"🥗"},{key:"stats",label:"Stats",icon:"📊"}].map(tab => (
          <button key={tab.key} onClick={() => setActiveTab(tab.key)} style={{ flex: 1, padding: "12px 4px", background: "transparent", border: "none", color: activeTab === tab.key ? "#00d4aa" : "#7c83a0", fontSize: 10, fontWeight: activeTab === tab.key ? 700 : 500, cursor: "pointer", borderBottom: activeTab === tab.key ? "2px solid #00d4aa" : "2px solid transparent" }}>
      ...
      <div style={{ padding: "16px 16px 100px" }}>
        ...
        {activeTab === "today" && (
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 20 }}>
              {[{ label: "Kcal cibles", value: targetCal, color: "#0099ff", sub: "objectif" }, { label: "Kcal mangees", value: totalCalEaten, color: totalCalEaten > targetCal ? "#ff4757" : "#00d4aa", sub: "aujourd'hui" }, { label: "Kcal brulees", value: calBurned, color: "#ff6b35", sub: "sport" }].map(card => (
                <div key={card.label} style={{ background: "rgba(255,255,255,0.04)", borderRadius: 16, padding: "14px 10px", border: "1px solid rgba(255,255,255,0.06)", textAlign: "center" }}>
        ...
        {activeTab === "week" && (
          <div>
            ...
            <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 20, border: "1px solid rgba(255,255,255,0.08)", padding: 16, marginBottom: 16 }}>
        ...
        {activeTab === "calories" && (
          <div>
            <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 20, border: "1px solid rgba(255,255,255,0.08)", padding: 20, marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
              {[{ key: "search", label: "Ajouter un aliment" }, { key: "myproducts", label: `Mes produits (${customFoods.length})` }].map(t => (
              ...
              <div>
                <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 20, border: "1px solid rgba(255,255,255,0.08)", padding: 16, marginBottom: 16 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                    ...
                    <button onClick={() => setShowAddProduct(true)} style={{ background: "linear-gradient(135deg,#00d4aa,#0099ff)", border: "none", color: "#fff", borderRadius: 10, padding: "6px 12px", cursor: "pointer", fontSize: 11, fontWeight: 700 }}>+ Nouveau produit</button>
                  </div>
                  <input value={searchFood} onChange={e => setSearchFood(e.target.value)} placeholder="Rechercher dans tous vos aliments..." style={inp} />
                  {searchFood && (
                    <div style={{ marginTop: 8, maxHeight: 220, overflowY: "auto", background: "rgba(0,0,0,0.4)", borderRadius: 10, border: "1px solid rgba(255,255,255,0.08)" }}>
                      {filteredFoods.length === 0 ? (
                        <div style={{ padding: 16, textAlign: "center", color: "#7c83a0", fontSize: 13 }}>
                          Aucun resultat —{" "}
                          <button onClick={() => setShowAddProduct(true)} style={{ background: "none", border: "none", color: "#00d4aa", cursor: "pointer", fontWeight: 700 }}>creer ce produit ?</button>
                        </div>
                      ) : filteredFoods.map(f => (
                        <button key={f.id} onClick={() => { setSelectedFood(f); setSearchFood(f.name); }} style={{ width: "100%", padding: "10px 14px", background: selectedFood?.id === f.id ? "rgba(0,212,170,0.15)" : "transparent", border: "none", color: "#e8eaf6", cursor: "pointer", textAlign: "left", display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 13, borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                          <span>{f.custom ? "⭐ " : ""}{f.name}</span>
                          <span style={{ color: "#ff6b35", fontWeight: 700, flexShrink: 0 }}>{f.cal} kcal</span>
                        </button>
                      ))}
                    </div>
                  )}
                  {selectedFood && (
                    <div style={{ marginTop: 12 }}>
                      <div style={{ background: "rgba(0,212,170,0.06)", border: "1px solid rgba(0,212,170,0.2)", borderRadius: 12, padding: 12, marginBottom: 10, fontSize: 12 }}>
                        ...
                        <div style={{ color: "#7c83a0" }}>Pour {foodQty} × {selectedFood.unit} : <span style={{ color: "#ff6b35", fontWeight: 700 }}>{Math.round(selectedFood.cal * foodQty)} kcal</span></div>
                        <div style={{ color: "#4a4f6a", marginTop: 2 }}>P:{(selectedFood.p * foodQty).toFixed(1)}g G:{(selectedFood.g * foodQty).toFixed(1)}g L:{(selectedFood.l * foodQty).toFixed(1)}g</div>
                      </div>
                      <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                        <div style={{ fontSize: 12, color: "#7c83a0" }}>Quantite :</div>
                        ...
                        <button onClick={addFoodToLog} style={{ flex: 1, padding: "10px", borderRadius: 10, background: "linear-gradient(135deg,#00d4aa,#0099ff)", border: "none", color: "#fff", fontWeight: 700, cursor: "pointer", fontSize: 13 }}>Ajouter au journal</button>
                      </div>
                    </div>
                  )}
                </div>
                ...
                {calorieLog.length > 0 && (
                  <div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                      ...
                      <button onClick={() => { if (window.confirm("Vider le journal ?")) setCalorieLog([]); }} style={{ background: "rgba(255,71,87,0.15)", border: "1px solid rgba(255,71,87,0.3)", color: "#ff4757", borderRadius: 8, padding: "4px 10px", cursor: "pointer", fontSize: 11 }}>Tout effacer</button>
                    </div>
                    {calorieLog.map(item => (
                      <div key={item.logId} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", background: "rgba(255,255,255,0.03)", borderRadius: 12, marginBottom: 8, border: "1px solid rgba(255,255,255,0.05)" }}>
                        <div><div style={{ fontSize: 13, fontWeight: 600 }}>{item.name}</div><div style={{ fontSize: 11, color: "#7c83a0" }}>x{item.qty} — P:{item.totalP}g G:{item.totalG}g L:{item.totalL}g</div></div>
                        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                          <div style={{ fontSize: 15, fontWeight: 800, color: "#ff6b35" }}>{item.totalCal}</div>
                          <button onClick={() => setCalorieLog(prev => prev.filter(i => i.logId !== item.logId))} style={{ background: "rgba(255,71,87,0.2)", border: "none", color: "#ff4757", borderRadius: 8, width: 28, height: 28, cursor: "pointer", fontSize: 14 }}>x</button>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div>
                ...
                {customFoods.length === 0 ? (
                  <div style={{ textAlign: "center", padding: "50px 20px", background: "rgba(255,255,255,0.03)", borderRadius: 20, border: "1px solid rgba(255,255,255,0.06)" }}>
                    ...
                    <div style={{ fontWeight: 700, marginTop: 16, marginBottom: 8 }}>Aucun produit enregistre</div>
                    <div style={{ fontSize: 13, color: "#7c83a0", marginBottom: 20 }}>Ajoutez vos produits du quotidien en reportant les valeurs de l'etiquette. Ils seront sauvegardes et disponibles pour toujours.</div>
                    <button onClick={() => setShowAddProduct(true)} style={{ background: "linear-gradient(135deg,#00d4aa,#0099ff)", border: "none", color: "#fff", borderRadius: 12, padding: "12px 24px", cursor: "pointer", fontWeight: 700 }}>Ajouter mon premier produit</button>
                  </div>
                ) : customFoods.map(f => (
                  <div key={f.id} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: "14px 16px", marginBottom: 10 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 11, color: "#7c83a0", marginTop: 3 }}>Pour {f.unit} — P:{f.p}g G:{f.g}g L:{f.l}g</div>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={{ fontSize: 18, fontWeight: 900, color: "#ff6b35" }}>{f.cal}<span style={{ fontSize: 10 }}> kcal</span></div>
                        <button onClick={() => { setCustomFoods(prev => prev.filter(x => x.id !== f.id)); showNotif("Produit supprime"); }} style={{ background: "rgba(255,71,87,0.2)", border: "none", color: "#ff4757", borderRadius: 8, width: 30, height: 30, cursor: "pointer", fontSize: 14 }}>🗑</button>
                      </div>
                    </div>
        ...
        {activeTab === "stats" && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
              {[{ label: "Metabolisme de base", value: Math.round(bmr), unit: "kcal/j", color: "#7c83a0", desc: "Au repos total" }, { label: "Depense totale", value: tdee, unit: "kcal/j", color: "#0099ff", desc: "Activite moderee" }, { label: "Objectif perte de poids", value: targetCal, unit: "kcal/j", color: "#00d4aa", desc: "Deficit de 20%" }, { label: "IMC", value: (weight / Math.pow(height / 100, 2)).toFixed(1), unit: "", color: "#ff6b35", desc: "Indice de masse corporelle" }].map(card => (
                <div key={card.label} style={{ background: "rgba(255,255,255,0.04)", borderRadius: 16, border: "1px solid rgba(255,255,255,0.06)", padding: "16px 14px" }}>
                  <div style={{ fontSize: 22, fontWeight: 900, color: card.color }}>{card.value}<span style={{ fontSize: 11 }}> {card.unit}</span></div>
      ...
      <style>{`* { -webkit-tap-highlight-color: transparent; } input::-webkit-inner-spin-button { opacity: 0.5; }`}</style>
    </div>
  );
}
